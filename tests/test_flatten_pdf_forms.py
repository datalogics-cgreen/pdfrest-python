...
from pdfrest.models._internal import PdfFlattenFormsPayload
...
def test_flatten_pdf_forms_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    output_id = str(PdfRestFileID.generate())

    payload_dump = PdfFlattenFormsPayload.model_validate(
        {"files": [input_file], "output": "flattened"}
    ).model_dump(mode="json", by_alias=True, exclude_none=True, exclude_unset=True)

    seen: dict[str, int] = {"post": 0, "get": 0}

    def handler(request: httpx.Request) -> httpx.Response:
        if request.method == "POST" and request.url.path == "/flattened-forms-pdf":
            seen["post"] += 1
    ...
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.flatten_pdf_forms(input_file, output="flattened")

    assert seen == {"post": 1, "get": 1}
    assert isinstance(response, PdfRestFileBasedResponse)
    assert response.output_file.name == "flattened.pdf"
    assert response.output_file.type == "application/pdf"
    assert str(response.input_id) == str(input_file.id)
    assert response.warning is None


def test_flatten_pdf_forms_request_customization(
    ...
    transport = httpx.MockTransport(handler)
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.flatten_pdf_forms(
            input_file,
            output="custom",
...
@pytest.mark.asyncio
async def test_async_flatten_pdf_forms_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        response = await client.flatten_pdf_forms(input_file)
...
@pytest.mark.asyncio
async def test_async_flatten_pdf_forms_request_customization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
    ...
    if isinstance(timeout_value, dict):
        assert all(
            component == pytest.approx(0.58) for component in timeout_value.values()
        )
    else:
        assert timeout_value == pytest.approx(0.58)


def test_flatten_pdf_forms_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    with (
        ...
        client.flatten_pdf_forms(png_file)
    ...
    with (
        ...
        client.flatten_pdf_forms([pdf_file, make_pdf_file(PdfRestFileID.generate())])
