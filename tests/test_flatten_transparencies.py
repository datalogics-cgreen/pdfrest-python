...
from pdfrest.models._internal import PdfFlattenTransparenciesPayload
...
def test_flatten_transparencies_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    output_id = str(PdfRestFileID.generate())

    payload_dump = PdfFlattenTransparenciesPayload.model_validate(
        {"files": [input_file], "output": "flattened", "quality": "high"}
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if (
            request.method == "POST"
            and request.url.path == "/flattened-transparencies-pdf"
        ):
            seen["post"] += 1
    ...
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.flatten_transparencies(
            input_file, output="flattened", quality="high"
        )

    assert seen == {"post": 1, "get": 1}
    assert isinstance(response, PdfRestFileBasedResponse)
    output_file = response.output_file
    assert output_file.name == "flattened.pdf"
    assert output_file.type == "application/pdf"
    assert response.warning is None
    assert str(response.input_id) == str(input_file.id)


def test_flatten_transparencies_request_customization(
...
@pytest.mark.asyncio
async def test_async_flatten_transparencies_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    payload_dump = PdfFlattenTransparenciesPayload.model_validate(
        {"files": [input_file], "quality": "medium"}
    ).model_dump(mode="json", by_alias=True, exclude_none=True, exclude_unset=True)
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        response = await client.flatten_transparencies(input_file)
...
@pytest.mark.asyncio
async def test_async_flatten_transparencies_request_customization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
...
def test_flatten_transparencies_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    with (
        ...
        client.flatten_transparencies(png_file)
