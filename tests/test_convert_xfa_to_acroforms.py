...
from pdfrest.models._internal import PdfXfaToAcroformsPayload
...
def test_convert_xfa_to_acroforms_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    output_id = str(PdfRestFileID.generate())

    payload_dump = PdfXfaToAcroformsPayload.model_validate(
        {"files": [input_file], "output": "acro"}
    ).model_dump(mode="json", by_alias=True, exclude_none=True, exclude_unset=True)

    seen: dict[str, int] = {"post": 0, "get": 0}

    def handler(request: httpx.Request) -> httpx.Response:
        if request.method == "POST" and request.url.path == "/pdf-with-acroforms":
            seen["post"] += 1
    ...
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.convert_xfa_to_acroforms(input_file, output="acro")

    assert seen == {"post": 1, "get": 1}
    assert isinstance(response, PdfRestFileBasedResponse)
    output_file = response.output_file
    assert output_file.name == "acro.pdf"
    assert output_file.type == "application/pdf"
    assert response.warning is None
    assert str(response.input_id) == str(input_file.id)


def test_convert_xfa_to_acroforms_request_customization(
    ...
    transport = httpx.MockTransport(handler)
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.convert_xfa_to_acroforms(
            input_file,
            output="custom",
...
@pytest.mark.asyncio
async def test_async_convert_xfa_to_acroforms_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        response = await client.convert_xfa_to_acroforms(input_file)
...
@pytest.mark.asyncio
async def test_async_convert_xfa_to_acroforms_request_customization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
...
def test_convert_xfa_to_acroforms_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    with (
        ...
        client.convert_xfa_to_acroforms(png_file)
...
@pytest.mark.asyncio
async def test_async_convert_xfa_to_acroforms_validation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    pdf_file = make_pdf_file(PdfRestFileID.generate(1))
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        with pytest.raises(ValidationError, match="Must be a PDF file"):
            await client.convert_xfa_to_acroforms(png_file)
