...
from pdfrest.models._internal import PdfToExcelPayload
...
def test_convert_to_excel_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    output_id = str(PdfRestFileID.generate())

    payload_dump = PdfToExcelPayload.model_validate(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.method == "POST" and request.url.path == "/excel":
            seen["post"] += 1
    ...
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.convert_to_excel(input_file, output="report")

    assert seen == {"post": 1, "get": 1}
    assert isinstance(response, PdfRestFileBasedResponse)
    output_file = response.output_file
    assert output_file.name == "report.xlsx"
    assert (
        output_file.type
        == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
...
def test_convert_to_excel_request_customization(
    ...
    transport = httpx.MockTransport(handler)
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.convert_to_excel(
            input_file,
            output="custom",
    ...
    assert isinstance(response, PdfRestFileBasedResponse)
    assert response.output_file.name == "custom.xlsx"
...
@pytest.mark.asyncio
async def test_async_convert_to_excel_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    payload_dump = PdfToExcelPayload.model_validate({"files": [input_file]}).model_dump(
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        response = await client.convert_to_excel(input_file)

    assert seen == {"post": 1, "get": 1}
    assert isinstance(response, PdfRestFileBasedResponse)
    assert response.output_file.name == "async.xlsx"
    assert (
...
@pytest.mark.asyncio
async def test_async_convert_to_excel_request_customization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
    ...
    assert isinstance(response, PdfRestFileBasedResponse)
    assert response.output_file.name == "async-custom.xlsx"
    timeout_value = captured_timeout["value"]
    assert timeout_value is not None
    if isinstance(timeout_value, dict):
        assert all(
...
def test_convert_to_excel_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    with (
        ...
        client.convert_to_excel(png_file)
