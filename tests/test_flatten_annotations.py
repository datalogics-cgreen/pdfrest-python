...
from pdfrest.models._internal import PdfFlattenAnnotationsPayload
...
def test_flatten_annotations_success(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    output_id = str(PdfRestFileID.generate())

    payload_dump = PdfFlattenAnnotationsPayload.model_validate(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if (
            request.method == "POST"
            and request.url.path == "/flattened-annotations-pdf"
        ):
            seen["post"] += 1
    ...
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.flatten_annotations(input_file, output="flattened")
...
def test_flatten_annotations_request_customization(
    ...
    transport = httpx.MockTransport(handler)
    with PdfRestClient(api_key=VALID_API_KEY, transport=transport) as client:
        response = client.flatten_annotations(
            input_file,
            output="custom",
...
@pytest.mark.asyncio
async def test_async_flatten_annotations_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    async with AsyncPdfRestClient(api_key=ASYNC_API_KEY, transport=transport) as client:
        response = await client.flatten_annotations(input_file)
...
@pytest.mark.asyncio
async def test_async_flatten_annotations_request_customization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
...
def test_flatten_annotations_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    ...
    with (
        ...
        client.flatten_annotations(png_file)
