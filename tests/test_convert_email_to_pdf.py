...
from pdfrest.models._internal import ConvertEmailToPdfPayload
...
def test_convert_email_to_pdf_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    payload_dump = ConvertEmailToPdfPayload.model_validate(
        {
            "files": [input_file],
            "output": "converted",
        }
...
def test_convert_email_to_pdf_request_customization(
...
def test_convert_email_to_pdf_validation_errors() -> None:
    email_file = make_source_file(
        str(PdfRestFileID.generate(1)),
        "message/rfc822",
        "message.eml",
    )
    ...
    with pytest.raises(ValidationError, match="Must be an RFC822 email file"):
        ConvertEmailToPdfPayload.model_validate({"files": [image_file]})

    with pytest.raises(
        ValidationError,
        match="List should have at most 1 item after validation",
    ):
        ConvertEmailToPdfPayload.model_validate({"files": [email_file, image_file]})


@pytest.mark.asyncio
async def test_async_convert_email_to_pdf_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_source_file(
