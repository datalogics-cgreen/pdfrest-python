...
async def test_async_translate_payload_accepts_valid_output_language(
    output_language: str,
) -> None:
    file_repr = make_pdf_file(PdfRestFileID.generate(1))
    payload = TranslatePdfTextPayload.model_validate(
...
@pytest.mark.asyncio
async def test_async_translate_pdf_text_json_success(
...
def test_translate_pdf_text_to_file_includes_pages_and_output(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(1))
    ...
    assert isinstance(response, TranslatePdfTextFileResponse)
    assert response.output_file.id == output_id
    assert response.output_file.name == "notes.md"
...
@pytest.mark.asyncio
async def test_async_translate_pdf_text_to_file_includes_pages_and_output(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("PDFREST_API_KEY", raising=False)
    input_file = make_pdf_file(PdfRestFileID.generate(2))
